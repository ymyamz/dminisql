...
func (master *Master) IndexDrop(input string, reply *string) error {
	...
	*reply = res
	...
	// 检查远程过程调用是否成功
	if call.Error != nil {
		fmt.Printf("%v region process index drop failed", ip)
		return call.Error
	}
...
func (master *Master) Complex_query_master(input string, reply *string) error {
	...
	//copy到master本地的数据库中
	for i := 0; i < size; i++ {
		found := master.TableCnt[tables[i]]
		fmt.Println(found)
		if found == 0 {
			master.Copy(tables[i], ip[i])
		}
		master.TableCnt[tables[i]] += 1
		fmt.Println(master.TableCnt[tables[i]])
	}

	//查询
	master.Query(input, reply)

	//清理
	for i := 0; i < size; i++ {
		master.TableCnt[tables[i]] -= 1
		if master.TableCnt[tables[i]] == 0 {
			input = "drop table " + tables[i]
			master.db.Exec(input)
		}
	}
...
// 将table复制到本地数据库中
func (master *Master) Copy(table string, ip string) error {
	fmt.Println("master copy.called")
	oldip := master.TableIP[table]
	rpcOldRegion := master.RegionClients[oldip]
